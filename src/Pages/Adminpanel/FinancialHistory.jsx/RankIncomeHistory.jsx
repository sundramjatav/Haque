...
import { rankIncomeHistory } from '../../../Api/admin.api';
import Loader from '../../../Components/Loader';

const RankIncomeHistory = () => {
  const title = "Rank Reward History";
  const headers = [
    "S.No",
    "ID",
    "Username",
  ...
  const fetchSelfIncomeDate = async () => {
    try {
      const response = await rankIncomeHistory();
      if (response.success) {
      ...
      <TableComponent
        ...
        renderRow={(item, index) => (
          <>
            ...
            <td className="border-r border-b border-text-white/40 p-2 md:p-3 text-center">{item?.reward?.title || "N/A"}</td>
            <td className="border-r border-b border-text-white/40 p-2 md:p-3 text-center">{item?.reward?.investment || "N/A"}</td>
...
export default RankIncomeHistory