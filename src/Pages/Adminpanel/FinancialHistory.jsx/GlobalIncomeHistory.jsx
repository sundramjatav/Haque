...
import { globalIncomeHistory } from '../../../Api/admin.api';
import Loader from '../../../Components/Loader';

const GlobalIncomeHistory = () => {
  const title = "Global Achiever History";
  const headers = [
    "S.No",
    "ID",
    "Username",
  ...
  const fetchSelfIncomeDate = async () => {
    try {
      const response = await globalIncomeHistory();
      if (response.success) {
      ...
      <TableComponent
        ...
        renderRow={(item, index) => (
          <>
            ...
            <td className="border-r border-b border-text-white/40 p-2 md:p-3 text-center">{item?.createdAt !== null ? new Date(item?.createdAt).toLocaleString(): "N/A"}</td>
